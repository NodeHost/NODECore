...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a NODE address (e.g. %1)").arg("NftP5oEuZyXPEX4geWDRyZhmcnRW6XsMvH"));
...
{
    // return if URI is not valid or is no nodehost: URI
...
{
    // Convert nodehost:// to nodehost:
    //
    //    Cannot handle this later, because nodehost:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open nodehost.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NodeHost.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for NodeHost.lnk
...
{
    return GetAutostartDir() / "nodehost.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a nodehost.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NodeHost\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the nodehost app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add nodehost app to startup item list
