...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::NODE, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (nodehost.conf).");
            QMessageBox::warning(this, "NodeHost Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#ffffff; bgcolor:#2b2d42;}\n a, span { font-family: monospace; }\n span.addr {color:#ffffff; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #8D99AE;}\n td.d0 {font-weight: bold; color:#ffffff;}\n h2, h3 { white-space:nowrap; color:#ffffff;}\n a { color:#2b2d42; text-decoration:none; }\n a.nav {color:#337ab7;}\n";
