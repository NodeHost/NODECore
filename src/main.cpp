...
#if defined(NDEBUG)
#error "NodeHost cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unode) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight >= 1 && nHeight <= 9999)
        nSubsidy = 1 * COIN;
    else if (nHeight >= 10000 && nHeight <= 49999)
        nSubsidy = 25 * COIN;
    else if (nHeight >= 50000 && nHeight <= 99999)
        nSubsidy = 15 * COIN;
    else if (nHeight >= 100000 && nHeight <= 249999)
        nSubsidy = 10 * COIN;
    else if (nHeight >= 250000 && nHeight <= 499999)
        nSubsidy = 5 * COIN;
    else if (nHeight >= 500000 && nHeight <= 999999)
        nSubsidy = 4 * COIN;
    else if (nHeight >= 1000000 && nHeight <= 1999999)
        nSubsidy = 3 * COIN;
    else if (nHeight >= 2000000 && nHeight <= 4999999)
        nSubsidy = 2 * COIN;
...
{
    RenameThread("nodehost-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (block.IsProofOfStake()) {
            ...
            if (!hostingFees.Validate(block, chainActive.Tip()->nHeight))
                return state.DoS(100, error("CheckBlock() : Invalid Hosting Fees payment"));
        }
    }
