...
/**
 * Fees smaller than this (in uNODE) are considered zero fee (for transaction creation)
...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        //check for min age
        if (GetAdjustedTime() - out.tx->GetTxTime() < Params().StakeMinAge())
            continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 NODE.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-node-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount nFees, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    hostingFees.Create(txNew, idxProofProofStake, pIndex0->nHeight);
