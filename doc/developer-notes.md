...
- [Development guidelines](#development-guidelines)
    - [General NodeHost Core](#general-nodehost-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NodeHost Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/NodeHost/NODECore/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nodehost
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nodehost --log_level=test_suite
$ valgrind -v --leak-check=full src/nodehostd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_nodehost.coverage/index.html`.
```

**Sanitizers**

NodeHost Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as NodeHost Core, where everyone uses
...
pay attention to for reviewers of NodeHost Core code.

General NodeHost Core
---------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NodeHost Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of NodeHost Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against NodeHost Core so that they can be integrated
...
file descriptors it will cause problems with NodeHost's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to NodeHost's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:NodeHost/NODECore.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `nodehost-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nodehost-cli` and the GUI debug console use this table to determine how to
